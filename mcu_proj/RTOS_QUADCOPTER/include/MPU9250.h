...
int16_t g_xyz[3];
long avr_roll, avr_pitch, avr_yaw;
...
void get_offset_value(void){//get 2000 times average offset value
    int cnt;
...
void get_YPR(void){
   ...
   //Subtract the offset values from the raw gyro values
    ...
    wait(300);  // wiat for next loop
    gyro_roll_input = (gyro_roll_input * 0.7) + ((g_xyz[0] / GYRO2DEGREE_PER_SEC) * 0.3);   //Gyro pid input is deg/sec.
    gyro_pitch_input = (gyro_pitch_input * 0.7) + ((g_xyz[1] / GYRO2DEGREE_PER_SEC) * 0.3);//Gyro pid input is deg/sec.
    gyro_yaw_input = (gyro_yaw_input * 0.7) + ((g_xyz[2] / GYRO2DEGREE_PER_SEC) * 0.3);      //Gyro pid input is deg/sec.

    //Gyro angle calculations . Note 0.0000611 = 1 / (250Hz x 65.5) =0.0000611
    angle_pitch += g_xyz[0] * 0.000611;                                   //Calculate the traveled pitch angle and add this to the angle_pitch variable
    angle_roll += g_xyz[1] * 0.000611;                                    //Calculate the traveled roll angle and add this to the angle_roll variable
