...
#define ANTI_WINDUP_LIMIT   20

//이중루프PID에서 제어할 변수들(Roll, Pitch, Yaw)//
float roll_target_angle = 0.0;
float roll_angle_in;
float roll_rate_in;
float roll_stabilize_kp = 8.0;  // angle kp     6.5
float roll_rate_kp = 0.30;  // angle rate kp setting   0.3
float roll_rate_ki = 0.00028;  // angle rate ki setting   0.37
float roll_rate_kd = 0.01;  // angle rate kd setting   0.032
...
float pitch_rate_in;
float pitch_stabilize_kp = 8.0;  // 1.8
float pitch_rate_kp = 0.30;  // 0.004 0379340
float pitch_rate_ki = 0.00028;  // 0.0035      --- 0.01 ~ 0.03
float pitch_rate_kd = 0.01;   // 0.0006
...
float yaw_rate_kp = 0.6;
float yaw_rate_ki = 0.02;
float yaw_rate_kd = 0.4;
float yaw_err_pre = 0;
...
void dualPID(float target_angle,
  ...
  *rate_err_pre = rate_error;
  ...
  if(*rate_iterm > ANTI_WINDUP_LIMIT){
      *rate_iterm = ANTI_WINDUP_LIMIT;
  }else if( *rate_iterm < -ANTI_WINDUP_LIMIT){
      *rate_iterm = -ANTI_WINDUP_LIMIT;
  }
...
//        motorA_PWM = throttle - yaw_output ;
//        motorB_PWM = throttle + yaw_output ;
//        motorC_PWM = throttle - yaw_output ;
//        motorD_PWM = throttle  + yaw_output ;
