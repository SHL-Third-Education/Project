...
void fpga_sock_config(char *port)
{
	...
	//fpga_serv_addr.sin_addr.s_addr = htonl("192.168.7.33");
	fpga_serv_addr.sin_addr.s_addr = inet_addr("192.168.7.33");
	fpga_serv_addr.sin_port = htons(atoi(port));
}

//void web_socket_config(int *sc, si *sa, int sa_size, char *port)
void web_socket_config(int *sc, si *sa, int sa_size, int port)
{
	web_serv_sock = socket(PF_INET, SOCK_STREAM, 0);
	...
	//web_serv_addr.sin_port = htons(atoi(port));
	web_serv_addr.sin_port = htons(port);

	if(bind(web_serv_sock, (sp)&web_serv_addr, sizeof(web_serv_addr)) == -1)
		err_handler("bind() error");

	if(listen(web_serv_sock, 1000) == -1)
		err_handler("listen() error");
...
void *fpga_rx(void *fd)
{
	...
	if(connect(fpga_sock, (sp)&fpga_serv_addr, sizeof(fpga_serv_addr)) == -1)
...
int main(int argc, char **argv)
{
	...
	/* thread 1 = Cell Phone Socket Recv
	   ...
	   thread 6 = Web Debugging Server(Recv)
	   Camera Proc                       */
	...
	thread_id = pthread_create(&p_thread[5], NULL, web_rx, NULL);
	if(thread_id < 0)
	{
		perror("Web Server RX thread create error: ");
		exit(0);
	}
